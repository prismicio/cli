...
7. Pass `--task-id` on every command for a user request, including read-only and exploratory commands such as `list`, `view`, `status`, and `whoami`. Run `npx prismic task-id` when the user asks for something new, and keep using that id until they ask for something else. Pass what they asked for as `--user-intent`.
