...
import { isPageType, removePrismicioRoutesConfigForType } from "./lib/pageType";
import { rejectIfNecessary } from "./lib/rejectIfNecessary";
import { syncPageTypeFiles } from "./lib/syncPageTypeFiles";
...
export const plugin = definePlugin<PluginOptions>({
	...
	setup({ hook }) {
		...
		hook("custom-type:create", async (data, context) => {
			...
			await syncPageTypeFiles(data.model, context);
		});
		...
		hook("custom-type:delete", async (data, context) => {
			// Remove route config for deleted page types
			if (isPageType(data.model)) {
				await removePrismicioRoutesConfigForType(
