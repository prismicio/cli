...
it("adds a text field to a slice", async ({ expect, project, prismic }) => {
	...
	const field = model.variations[0].primary.subtitle;
	expect(field).toMatchObject({ type: "Text" });
});

it("adds a text field to a custom type", async ({ expect, project, prismic }) => {
	...
	const model = JSON.parse(await readFile(modelPath, "utf-8"));
	const field = model.json.Main.subtitle;
	expect(field).toMatchObject({ type: "Text" });
});
