...
it("adds a rich text field to a slice", async ({ expect, project, prismic }) => {
	...
	expect(exitCode).toBe(0);
	expect(stdout).toContain("Field added: my_richtext");

	const model = JSON.parse(await readFile(modelPath, "utf-8"));
	const field = model.variations[0].primary.my_richtext;
	expect(field).toMatchObject({ type: "StructuredText" });
});

it("adds a rich text field to a custom type", async ({ expect, project, prismic }) => {
	...
	const model = JSON.parse(await readFile(modelPath, "utf-8"));
	const field = model.json.Main.my_richtext;
	expect(field).toMatchObject({ type: "StructuredText" });
});
