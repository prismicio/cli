...
it("adds a boolean field to a slice", async ({ expect, project, prismic }) => {
	...
	const field = model.variations[0].primary.my_field;
	expect(field).toMatchObject({ type: "Boolean" });
});

it("adds a boolean field to a custom type", async ({ expect, project, prismic }) => {
	...
	const model = JSON.parse(await readFile(modelPath, "utf-8"));
	const field = model.json.Main.is_active;
	expect(field).toMatchObject({ type: "Boolean" });
});
