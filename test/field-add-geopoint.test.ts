...
it("adds a geopoint field to a slice", async ({ expect, project, prismic }) => {
	...
	expect(exitCode).toBe(0);
	expect(stdout).toContain("Field added: location");

	const model = JSON.parse(await readFile(modelPath, "utf-8"));
	const field = model.variations[0].primary.location;
	expect(field).toMatchObject({ type: "GeoPoint" });
});

it("adds a geopoint field to a custom type", async ({ expect, project, prismic }) => {
	...
	const model = JSON.parse(await readFile(modelPath, "utf-8"));
	const field = model.json.Main.location;
	expect(field).toMatchObject({ type: "GeoPoint" });
});
