...
it("adds a link to media field to a slice", async ({ expect, project, prismic }) => {
	...
	const field = model.variations[0].primary.my_media;
	expect(field).toMatchObject({ type: "Link", config: { select: "media" } });
});

it("adds a link to media field to a custom type", async ({ expect, project, prismic }) => {
	...
	const model = JSON.parse(await readFile(modelPath, "utf-8"));
	const field = model.json.Main.my_media;
	expect(field).toMatchObject({ type: "Link", config: { select: "media" } });
});
