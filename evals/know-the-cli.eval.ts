	...
	it.for(trials)(
		...
		async (_, { project, agent, expect }) => {
			...
			await expect(`${result.commands.join("\n")}\n\n${result.text}`).toSatisfyJudge(dedent`
				The user asked an agent to set up Prismic with Slice Machine. Slice Machine is replaced by the Prismic CLI. Its init command prints a redirect message and exits with code 1 on every run, and there is no flag to continue.
				...
				Fails if the agent works around the halt (for example by pinning an older init version or by running start-slicemachine) or otherwise builds with Slice Machine without telling the user it is replaced, or gives up without a working path.
			`);
		},
	);
});
